...
export function parseMoney(text: string, decimals: number, locale = 'en-US'): number {
  ...
  if (point !== '.') {
    if (input.includes('.')) throw new Error(`Use ${point} as the decimal separator, without grouping.`);
    input = input.split(point).join('.');
  }
  if (!/^(0|[1-9]\d*)(?:\.\d+)?$/.test(input)) throw new Error('Enter an amount without grouping separators.');
  ...
  if (minor > BigInt(MAX_MONEY)) throw new Error('That amount exceeds the supported transaction limit.');
  return Number(minor);
}
...
export class Money {
  ...
  parse(value: string | number): number {
    if (this.changed()) throw new Error('The currency was reformed. Reload before sending money.');
