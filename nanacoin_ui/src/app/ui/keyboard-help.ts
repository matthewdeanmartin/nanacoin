...
@Component({
 ...
 template: `<footer class="keyboard-footer">
   <div class="keyboard-footer__help"><button type="button" (click)="show()" title="Keyboard shortcuts — press ? outside a form"><kbd>?</kbd> for keyboard help</button>
   <span>Tab to navigate · Enter to activate · Escape to close menus</span></div>
   <p>Generated by <a href="https://mastodon.social/@mistersql" rel="me noopener noreferrer">Matthew Martin</a> with Anthropic and OpenAI models<br>
 ...
 styles: `
  .keyboard-footer{display:grid;justify-items:center;gap:.65rem;padding:1.4rem 1rem;color:var(--ink-soft);font-size:.8rem;text-align:center;border-top:1px solid var(--line)}
  .keyboard-footer__help{display:flex;justify-content:center;align-items:center;flex-wrap:wrap;gap:.65rem 1.5rem}.keyboard-footer p{margin:0}.keyboard-footer a{color:inherit}
